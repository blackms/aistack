...
/** Pinned version of @sentry/mcp-server. Bump deliberately, never `latest`. */
export const SENTRY_MCP_VERSION = '0.35.0';
...
export const sentryAdapter: BattlePackAdapter<SentryIntegrationConfig> = {
  ...
  build(config: SentryIntegrationConfig): BattlePackEntry {
    const pkg = config.packageName ?? `@sentry/mcp-server@${SENTRY_MCP_VERSION}`;
