...
- **Marketplace plugin bundle (AIG-629)**: ASSENTE in main (solo branch). aistack non si distribuisce ancora *come* plugin Claude Code — proprio la strada raccomandata.
...
|---|---|---|---|---|---|
| 1 | **Distribuire aistack come plugin Claude Code** (mergiare AIG-629: manifest + skills + commands) | "Voglio durabilità/federazione/governance senza abbandonare il flusso Claude Code" | Alto | **Basso** | Bundle già scritto sul branch `aig-629`; basta mergiarlo. Sblocca distribuzione/versioning nativi (Plugins/Marketplace) ed è la strada esplicitamente raccomandata: distribuirsi *come* plugin, non costruire un package manager. |
